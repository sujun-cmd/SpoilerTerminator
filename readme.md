🎬 SpoilerTerminator
Automatic Spoiler Detection and Spoiler-Free Rewriting via Embedding-Based Weak Supervision
Course Project for STATS 507, University of Michigan
📖 Overview
SpoilerTerminator is an end-to-end data science pipeline designed to detect and neutralize spoilers in movie and book reviews.
The core challenge in spoiler detection is Label Noise (Weak Supervision): existing datasets often provide document-level labels (e.g., "This review contains a spoiler"), but individual sentences within the review might be safe (e.g., "The acting was great").
This project introduces a novel Teacher-Student Framework to solve this:
1. Vector Teacher: A Logistic Regression model trained on MPNet embeddings identifies the semantic "direction" of spoilers (e.g., death, plot twists) to clean noisy data.
2. RoBERTa Student: A fine-tuned RoBERTa model trained on the cleaned high-quality data via Self-Training.
3. Generative Despoiler: A quantized Qwen-2.5-14B LLM that rewrites detected spoilers into vague, suspenseful teasers.
🚀 Pipeline Architecture
graph LR
   A[Raw Noisy Data] -->|Sentence Embedding| B[MPNet-Base]
   B --> C[LR Teacher Model]
   C -->|Filter: Score > 0.75 or < 0.4| D[Cleaned Data]
   D -->|Fine-tuning| E[RoBERTa Student]
   E -->|Detection| F{Is Spoiler?}
   F -->|Yes| G[Qwen LLM Despoiler]
   F -->|No| H[Keep Original]
   G --> I[Safe Teaser]

📂 Repository Structure
SpoilerTerminator/
├── train_vector.py          # Step 1: Train the LR Teacher using MPNet embeddings
├── eval_vector.py           # Step 1.5: Evaluate the Teacher's direction finding
├── run_self_train.py        # Step 2: Teacher cleans data -> Trains RoBERTa Student
├── compare_models_kaggle.py # Step 3: Cross-Domain Evaluation (IMDB -> Goodreads)
├── despoiler.py             # Module: LLM-based rewriter class (Qwen-14B-Int4)
├── main.py                  # Step 4: End-to-end inference pipeline
├── reviews.txt              # Sample input file
└── *.slurm                  # Slurm submission scripts for HPC clusters

🛠️ Installation & Requirements
This project is optimized for Linux Servers with NVIDIA GPUs (tested on V100 16GB/32GB).
1. Clone the repository
git clone [https://github.com/sujun-cmd/SpoilerTerminator.git](https://github.com/sujun-cmd/SpoilerTerminator.git)
cd SpoilerTerminator

2. Environment Setup (Using conda/venv)
# Recommended Python version: 3.10+
python -m venv hf_env
source hf_env/bin/activate

# Install dependencies
pip install torch torchvision torchaudio
pip install transformers datasets sentence-transformers
pip install scikit-learn accelerate bitsandbytes
pip install nltk

3. Download NLTK Data
import nltk
nltk.download('punkt')
nltk.download('punkt_tab')

🏃‍♂️ Usage
1. Train the Vector Teacher
Extracts semantic directions of spoilers from noisy IMDB data.
python train_vector.py

2. Self-Training (Clean & Fine-tune)
Uses the Teacher to filter the dataset and trains the RoBERTa Student.
python run_self_train.py

3. Cross-Domain Evaluation (Optional)
Tests the models on the Goodreads book review dataset to verify robustness.
python compare_models_kaggle.py

4. Run the Full Pipeline (Detect & Rewrite)
Process reviews.txt and generate despoiled_reviews.txt.
python main.py

📊 Experimental Results
1. Robustness against Label Noise (IMDB)
Our Teacher-Student approach significantly outperformed the baseline. Notably, the model was able to correct ground truth errors in the original dataset.
Model
	F1 Score
	Recall
	Precision
	Note
	Baseline (Naive)
	0.26
	Low
	Low
	Overfitted to noise
	RoBERTa (Ours)
	0.49
	0.69
	0.40
	Corrects mislabeled data
	Example of Correction:
   * Input: "Why didn't Obi-Wan kill Anakin?"
   * Original Label: Safe (Wrong)
   * Our Model: Spoiler 1.0 (Correct)
2. Cross-Domain Generalization (Books)
Tested on 10,000 sentences from Goodreads (Zero-shot transfer).
   * LR Teacher: Achieved 0.84 Recall, proving that "spoiler semantics" (death, reveal) are universal across domains.
   * RoBERTa Student: Achieved 0.66 Accuracy, showing better precision in filtering non-spoiler noise.
3. Generative Rewriting (Qwen-2.5-14B)
We used 4-bit quantization to fit Qwen-14B on a single V100 GPU.
Original Spoiler
	LLM Despoiled Version
	"Bruce Willis is actually a ghost in the end."
	"There is a mind-blowing revelation about Bruce Willis's true nature."
	"She dies in the car accident."
	"A harrowing car accident leaves one character's fate hanging in the balance."
	📝 Citation
If you find this code useful, please cite our project report:
@article{Su2025SpoilerTerminator,
 title={SpoilerTerminator: Automatic Spoiler Detection and Rewriting via Weak Supervision},
 author={Jun Su},
 journal={STATS 507 Final Project, University of Michigan},
 year={2025}
}

📧 Contact
Jun Su - sujun@umich.edu